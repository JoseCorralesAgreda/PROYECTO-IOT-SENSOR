...
Contrato para implementar la práctica educativa de JOSE FRANZ. Es diseño documental; no acredita compilación, pruebas ni montaje. El software puede desarrollarse con entradas simuladas. La integración física sigue condicionada por P-01 a P-03 del [PRD](../../prds/prd-indicador-distancia-2026-09-07/prd.md).
...
Los campos **Binds**, **Prevents** y **Rule** indican respectivamente qué vincula cada decisión, qué incompatibilidad evita y la regla exigible. `[ADOPTED]` identifica instrucciones del usuario o realidad existente; `[SUPUESTO]` identifica elecciones propuestas por la vía rápida.
...
- **Rule:** [ADOPTED] C++ orientado a objetos, código y comentarios en inglés, documentación en español; comentarios solo si aclaran intención no evidente. [SUPUESTO] `main.cpp` compone objetos de vida estática y coordina; `UltrasonicSensor` produce lecturas; `DistanceIndicator` es único dueño de color y fase; `LedDriver` aplica salidas sin reinterpretarlas. `EchoCapture` valida secuencias temporales dentro del adaptador y se prueba sin hardware. El código de referencia del informe no sustituye estas reglas de integración.
...
- **Rule:** [SUPUESTO S-04] iniciar como máximo una adquisición cada 100 ms medidos entre inicios reales; sin ráfagas para recuperar ciclos atrasados. Mantener Trigger bajo inicialmente y emitir un pulso alto de al menos 10 µs; solo se admite esa breve espera de disparo, nunca `pulseIn` ni esperas del eco en el bucle.
...
- **Rule:** [ADOPTED] para una lectura válida: menor de 10 cm rojo; desde 10 hasta menos de 30 amarillo; desde 30 verde. Solo uno encendido. [SUPUESTO S-01] rango válido inclusivo de 2 a 400 cm; toda lectura inválida tiene prioridad. [SUPUESTOS S-02/S-03] arranque inválido; timeout inválido; comenzar fase con los tres encendidos al iniciar y en cada transición válida→inválida; alternar cada 250 ms sin reiniciar por otra inválida. Al recuperarse, aplicar el color en la misma iteración que consume la lectura. Saltos del reloj conservan fase según medias fases transcurridas, sin reproducir transiciones atrasadas. `LedOutput` contiene tres booleanos `red`, `yellow`, `green`; `true` significa LED encendido, y el adaptador activo alto escribe los tres GPIO en cada aplicación. El bucle actualiza salidas en cada iteración; objetivo propuesto de retraso máximo 10 ms, a medir en hardware.
...
- **Rule:** [ADOPTED] pruebas unitarias sobre las mismas clases del firmware. [SUPUESTO] entorno anfitrión de PlatformIO y Unity, sin Arduino en las pruebas del núcleo; reloj y flancos inyectados. Cubrir 1,99/2/9,99/10/29,99/30/400/400,01 cm, ausencia, negativos, NaN e infinito; exclusión, arranque, recuperación, inválidas sucesivas, marcas 249/250/499/500 ms y desbordamiento. Para captura: flancos sin subida, orden correcto, límite de timeout, Echo alto inicial, ecos tardíos, consumo único y ausencia de novedades. Compilar posteriormente el firmware real además de ejecutar las unitarias; pruebas físicas de GPIO y tiempos son una evidencia separada. Registrar versiones efectivamente usadas y salida de los comandos; no declarar aprobado un ensayo no ejecutado.
...
- **Rule:** [ADOPTED] solo ESP32, sensor de 5 V, tres LEDs y una resistencia de 220 Ω por LED; sin conectividad ni funciones adicionales. GPIO propuestos: Trigger 18, Echo 19, rojo 25, amarillo 26, verde 27; verificar placa y cableado antes de usarlos. Echo permanece sin conexión al GPIO mientras no se demuestre compatibilidad con su límite de 3,6 V. No implementar una conexión directa de 5 V ni agregar adaptación por inferencia. Desarrollo y pruebas unitarias en ordenador; firmware en la única placa mediante carga local de PlatformIO. Sin backend, almacenamiento ni actualización remota. Reiniciar no conserva estado y vuelve a la fase inicial de AD-4. Validación física suspendida hasta resolver los pendientes eléctricos.

## Base tecnológica verificada

Semilla para implementación, aún no aplicada a `platformio.ini`. Se conserva el entorno existente; las versiones instaladas se comprobaron por sus manifiestos, sin actualizar dependencias.

| Elemento | Versión o evidencia |
|---|---|
| C++ | C++17 propuesto; verificar bandera y compilación al implementar |
| Plataforma Espressif32 | 7.1.1 instalada; fijar `espressif32@7.1.1` al implementar |
| Arduino ESP32, paquete PlatformIO | `3.20017.241212+sha.dcc1105b` instalado; registrar/fijar resolución al implementar |
| Placa | `esp32doit-devkit-v1`, declarada en configuración local |
| Pruebas | Unity con entorno anfitrión de PlatformIO; versiones se fijan antes de ejecutar, no instaladas ni validadas en esta entrega |
...
La ubicación de definiciones pequeñas puede simplificarse al implementar; los límites de dependencia permanecen.
...
- **Supuestos S-01 a S-04:** heredados del PRD, propuestos y pendientes de aceptación; no convertirlos en resultados medidos. Software con simulación puede avanzar; cierre físico no.
- **Implementación:** fijar versiones del entorno de pruebas y compilador anfitrión cuando se preparen las pruebas, antes de su primera ejecución; no cambian contratos entre componentes. Fijar paquetes y C++17 en configuración, comprobar compilación, detalles de sección crítica y tiempo máximo de ISR en el entorno real.
- **Evaluación educativa:** incorporación de esta arquitectura al informe existente pendiente de preferencia del usuario. El informe conserva su condición de propuesta y resultados no ejecutados.

La arquitectura permanece en borrador condicionado por los pendientes físicos y de aceptación; no modifica firmware ni configuración en esta entrega.
